-- Make the order save (and the אישור אספקה confirm) ATOMIC with its money rows.
--
-- Until now POST /api/orders/update did three separate writes: this SECURITY
-- DEFINER RPC (order + items + stock — committed on return), THEN a payments
-- insert, THEN a refunds insert, both through the caller's RLS-bound client.
-- When the payments insert was rejected (2026-09-15: worker RETURNING/SELECT
-- RLS, see 20260915145206_fix_worker_payment_select_returning_rls.sql) the
-- order had already been closed to 'delivered' and its stock consumed, while
-- the ₪680 the driver collected was never recorded and could not be retried.
--
-- Fix: update_sales_order now takes the fully-built payment / refund rows
-- (p_payments / p_refunds, jsonb arrays shaped by lib/payments.ts's
-- buildPaymentInsert) and inserts them inside the SAME transaction, then returns
-- the new ids so the route can still auto-issue Morning receipts. A failed money
-- row rolls the whole save back. order_id, business_domain and recorded_by are
-- forced server-side from the RPC's own arguments, never trusted from the rows.
--
-- The 14-, 15- and 16-parameter overloads left behind by earlier
-- create-or-replace migrations are dropped: the route is the only caller and
-- always passes every named argument, and a single definition means PostgREST
-- can never hit "could not choose the best candidate function". Because the two
-- new parameters default to '[]', a route deployed BEFORE this migration keeps
-- working (it resolves to this function; the payments insert then runs the old
-- way), and a route deployed AFTER it simply passes the rows in. Return type is
-- now jsonb {order_id, payment_ids, refund_ids}; the old code tolerated a
-- non-string result already.
--
...
drop function if exists public.update_sales_order(
  uuid, uuid, timestamptz, text, numeric, numeric, numeric, text, uuid, text, jsonb, text, date, timestamptz, date
);
drop function if exists public.update_sales_order(
  uuid, uuid, timestamptz, text, numeric, numeric, numeric, text, uuid, text, jsonb, text, date, timestamptz, date, uuid
);
drop function if exists public.update_sales_order(
  uuid, uuid, timestamptz, text, numeric, numeric, numeric, text, uuid, text, jsonb, text, date, timestamptz, date, uuid, jsonb, jsonb
);
