-- Adds an OPTIONAL customer-requested delivery date to orders — distinct from
-- delivery_confirmed_at, which already means "when delivery was confirmed as
-- done" (set only by the אישור אספקה flow). This one is forward-looking: "when
-- the customer asked for it," left null in the normal case.
--
-- Re-bases create_sales_order/update_sales_order on the bodies from
-- 20260820201352_fix_partial_delivery_and_custom_lines_merge.sql (the latest
-- migration touching them) — copying from an older version would silently
-- regress the nullable-product_id/custom-line support that migration added,
-- since CREATE OR REPLACE restates the whole function body rather than diffing.
--
...
grant execute on function public.create_sales_order(
  uuid, timestamptz, text, numeric, numeric, numeric, text, uuid, text, jsonb, text, date, boolean, date
) to authenticated;
