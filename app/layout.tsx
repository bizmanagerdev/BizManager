    ...
    <html lang="he" dir="rtl" suppressHydrationWarning>
      <head>
        {/* Self-heal a stale service-worker cache serving an old page whose JS chunk
            ...
            ALSO gated on NEXT_PUBLIC_E2E_TEST (2026-09-16): e2e CI now runs a real
            production build instead of `next dev` (see playwright.config.ts), so
            this is the first time this script is ever actually rendered during a
            CI run — same failure shape as the dev-mode case above, just triggered
            by any transient static-asset hiccup on a loaded CI runner instead of a
            Fast Refresh swap. See lib/ui/auto-recover.ts's matching guard. */}
        {process.env.NODE_ENV === "production" && process.env.NEXT_PUBLIC_E2E_TEST !== "1" ? (
          <script
