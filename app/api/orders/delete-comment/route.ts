...
/**
 * Delete one comment from an order. Comments live as a small text log inside the
 * order's `notes` field (no schema change): read notes, locate the target
 * comment by content, drop it, and write the log back.
 */
export async function POST(req: Request) {
  try {
    ...
    const body = (await req.json()) as { order_id?: string; target?: OrderComment };
    const orderId = typeof body.order_id === "string" ? body.order_id : "";
    const target = body.target;

    if (!orderId || !target) {
      return NextResponse.json({ error: "Missing order_id or target" }, { status: 400 });
    }
    ...
    if (index === -1) {
      return NextResponse.json({ error: "התגובה כבר נמחקה. רעננו ונסו שוב." }, { status: 409 });
    }

    const nextComments = comments.filter((_, i) => i !== index);
