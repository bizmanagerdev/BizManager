...
export default function CcFeeRateCard({ initialRate }: { initialRate: number }) {
  ...
  function save() {
    ...
    scheduleDeferredAction({
      key: "settings:cc-fee-rate",
      message: "שיעור עמלת הסליקה עודכן",
      ...
      onCommit: async () => {
        const res = await fetch("/api/settings/cc-fee", {
          ...
          body: JSON.stringify({ cc_fee_rate: targetNumeric / 100 }),
        });
        const json = (await res.json().catch(() => null)) as { cc_fee_rate?: number; error?: string } | null;
        if (!res.ok) return { ok: false, error: toHebrewError(json?.error, "שמירת שיעור העמלה נכשלה.") };
        const next = typeof json?.cc_fee_rate === "number" ? Math.round(json.cc_fee_rate * 10000) / 100 : targetNumeric;
