              ...
              >
                ...
                <div className="space-y-1">
                  {dayProjects.slice(0, 2).map((p) => (
                    <div key={`${p.id}-${day.toISOString()}`} className="rounded bg-primary/10 px-1.5 py-1 text-[11px]">
                      <div className="truncate font-medium">{p.name}</div>
                      <div className="truncate text-muted-foreground">{p.customerName}</div>
                    </div>
                  ))}
                  {dayProjects.length > 2 ? (
                    <div className="text-[11px] text-muted-foreground">+{dayProjects.length - 2} נוספים</div>
                  ) : null}
                </div>
              </div>
            );
