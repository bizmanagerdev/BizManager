...
/**
 * Scroll to and flash the on-screen element carrying `data-focus-id="<id>"`,
 * retrying for a few seconds while the list it lives in is still rendering.
 * The same effect the `?focus=` deep link produces, callable from a page that
 * just changed something and wants to show WHERE it landed (e.g. a payment that
 * moved to another day). Returns a cancel function.
 */
export function flashFocusTarget(
  focusId: string,
  { timeoutMs = LOOKUP_TIMEOUT_MS }: { timeoutMs?: number } = {}
): () => void {
  let cancelled = false;
...
export default function FocusHighlighter() {
  ...
  useEffect(() => {
    if (!focusId) return;
    return flashFocusTarget(focusId);
  }, [focusId]);
