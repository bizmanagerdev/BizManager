...
/**
 * Waze brand mark — the official Waze face glyph (from Simple Icons). Rendered
 * as a solid `currentColor` fill so it sits alongside the lucide icons used
 * across the app while staying recognizable as Waze. Shown next to any
 * *displayed* address to signal that tapping it opens Waze navigation (see
 * {@link AddressLink}). Accepts the same props as a lucide icon (className /
 * size).
 */
export interface WazeIconProps extends React.SVGProps<SVGSVGElement> {
  size?: number | string;
}

export function WazeIcon({ size = 24, className, ...props }: WazeIconProps) {
  // Padded, square viewBox insets the glyph so its head (flush at y=0 in the
  // source path) keeps clearance and isn't clipped by tight line boxes such as
  // the compact deliveries rows.
  return (
    <svg
